	...
	*/
	...
	private async setupAuthorizationHeader(
		...
		return new Promise<AxiosRequestConfig>((resolve, reject) => {
			let id;
			if (localStorage.getItem("simple.result")) {
				id = JSON.parse(localStorage.getItem("simple.result") || "").idToken;
				if (!config) {
				...
				console.error("Error from getAuthToken: ");
				window.location.href = "/signin";
			}
			// const authTokenRequest = {
			// 	successCallback: (token: string) => {
			// 		console.log("retrive getAuthToken", token);
			// 		if (!config) {
			// 			config = axios.defaults;
			// 		}
